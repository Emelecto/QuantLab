    ...
    try {
      ...
      if (!resp.ok) {
        ...
        if (resp.status === 451 && hostKey !== BINANCE_HOST_KEYS[BINANCE_HOST_KEYS.length - 1]) {
          continue; // geobloqueo; probamos el siguiente mirror
        }
      ...
      if (hostKey === BINANCE_HOST_KEYS[BINANCE_HOST_KEYS.length - 1]) {
        throw e instanceof Error ? e : new Error("binance: error");
      }
      // mirror falló, probamos el siguiente
    }
  }
  throw new Error("binance: todos los mirrors fallaron");
}
...
export async function GET(req: NextRequest) {
  ...
  return new NextResponse(csv, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${safeName}_${interval}_${rows.length}.csv"`,
